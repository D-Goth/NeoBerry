...
Chart.register({
    ...
    afterDraw: function (chart) {
        ...
        if (!chart.options.plugins.customNetworkLabel) {
            // Appliqué uniquement si ce n'est pas une jauge réseau
            ctx.save();
...
function updateBatteryGauge(data) {
    ...
    if (percent < 25)
        color = '#ff0000'; // Rouge (<25%)
    else if (percent < 50)
        color = '#ff8000'; // Orange (25-49%)
...
function scanDevices() {
    ...
    // Affichage temporaire pendant le scan
    ...
    fetch('/bluetooth/scan')
        ...
        .then((data) => {
            ...
            // Nettoyer et injecter les périphériques
            list.innerHTML = '';
...
function updateSelectedDeviceUI(list) {
...
function renderPairedDevices(devices) {
    ...
    for (let i = 0; i < Math.max(minLines, total); i++) {
        ...
        if (i < total) {
            ...
            li.addEventListener('click', () => {
                document.querySelectorAll('#paired-devices-list li').forEach((el) => el.classList.remove('selected'));
